    ...
    const onFilesError = (error, file) => {
        if (error.code === 2) {
            ...
            MySwal.fire({
                position: "center",
                icon: "error",
                title: "El tipo de archivo no es valido. Elige un tipo de archivo permitido",
                showConfirmButton: false,
                timer: 2500,
            });
        }
            ...
            {isFileSelected &&
                <div className="upload_dropzone_file-info">
                    <div className="file-info_wrapper">
                        ...
                        <span
                            className="upload_dropzone_file-info_text">
                            <b>{currFile?.name && currFile.name}</b>
                            <p>{currFile?.sizeReadable && currFile.sizeReadable}</p>
                            <small>{currFile?.lastModifiedDate && currFile.lastModifiedDate.toDateString()}</small>
                        </span>
                    </div>
                    {
                        isFileSelected ?
