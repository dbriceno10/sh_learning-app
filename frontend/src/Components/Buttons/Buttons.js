...
export default function Button({
    ...
    if ((!type || type === '')) {
        if ((typeof link === 'string' && typeof btnVariant === 'string')) {
            ...
            if (icon && typeof icon === 'string') {
                ...
                if (btnVariant === 'flat-icon') {
                    if (link.startsWith('/')) {
                        ...
                        return (
                            <button className='button' onClick={onClick}>
                                <Link className="button-flat icon"
                                    to={link !== '' ? link : ''}>
                        ...
                        return (
                            <button className='button button-flat icon' onClick={onClick} type={'button'}>
                                {text ? <span>{text}</span> : null}
                                <Icon icon={icon} />
                            </button>
                        )
                    ...
                    if (link.startsWith('/')) {
                        return (
                            <button className='button' onClick={onClick}>
                                <Link className="button-raised icon"
                                    to={link !== '' && link}>
                                    {text ? <span>{text}</span> : null}
                                    <Icon icon={icon} />
                                </Link>
                        ...
                        return (
                            <button className='button button-raised icon' onClick={onClick} type={'button'}>
                                {text ? <span>{text}</span> : null}
                                <Icon icon={icon} />
                            </button>
                        )
                    ...
                    if (link === '') {
                        return (
                            <button className='button button-round tooltip' onClick={onClick} type={'button'}>
                                <Icon icon={icon} />
                                {(tooltip && tooltip !== '')
                    ...
                    console.error('Button variant does not exist. Name might be wrong or misspelled');
                    return null;
                }
            ...
            console.error(`Link and Variant attributes must be strings only for Button component`);
            return null;
        }
