...
export default function shoppingReducer(
  ...
  switch (type) {
    case ADD_TO_CART: {
      ...
      if (myCourses) {
        // Si existe ese array, le pusheo el nuevo objeto (payload)
        myCourses.push(payload);
        localStorage.setItem("cart", JSON.stringify(myCourses));
      } else {
        // Si no existe, seteo el localStorage con un array de objetos
    ...
    case REMOVE_ONE_FROM_CART: {
      ...
      // console.log(payload);
      let newItems = state.localStorageCart.filter(
